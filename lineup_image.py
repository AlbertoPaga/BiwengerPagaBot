...
def _font(size: int, bold: bool = False):
    candidates = (["/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", "/usr/share/fonts/truetype/liberation2/LiberationSans-Bold.ttf"] if bold else ["/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", "/usr/share/fonts/truetype/liberation2/LiberationSans-Regular.ttf"])
...
def _normalizar_report(report: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if position not in POSITION_LABELS:
        return None
    return {"id": player.get("id"), "name": str(player.get("name") or "Jugador"), "position": position, "position_label": POSITION_LABELS[position], "alt_positions": player.get("altPositions") or [], "points": report.get("points")}
...
def alineacion_confirmada(game: dict[str, Any], now: datetime | None = None) -> bool:
    """Devuelve True cuando Biwenger ya marca los onces como confirmados.

    Si la API todavía no expone ``initialLineups``, usamos la hora del partido
    como respaldo: a partir del comienzo del encuentro se considera que deben
    mostrarse los onces reales si están disponibles.
    """
...
def obtener_alineacion_mostrable(game: dict[str, Any], team_key: str, now: datetime | None = None) -> tuple[list[dict[str, Any]], bool]:
    """Selecciona automáticamente el 11 real o el 11 posible.

    Antes de confirmarse usa ``reports`` (los posibles). Cuando ``initialLineups``
    es True, o ya ha llegado la hora del partido, usa los datos disponibles de
    ``reports`` como alineación real. La segunda posición del retorno indica si
    debe mostrarse la columna de puntos.
    """
    team = game.get(team_key, {}) if isinstance(game, dict) else {}
    confirmed = alineacion_confirmada(game, now=now)
    return normalizar_alineacion(team), confirmed
