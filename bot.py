...
def _league_id(liga):
    return liga.get("id") if isinstance(liga, dict) else None


def _league_name(liga):
    return (
        liga.get("name", "Liga sin nombre")
        if isinstance(liga, dict)
        else "Liga sin nombre"
    )
...
async def mostrar_selector_liga(update):
    ...
    for liga in ligas:
        liga_id = _league_id(liga)
        liga_nombre = _league_name(liga)
...
async def elegir_liga(update, context):
    ...
    try:
        ...
        liga_id = query.data.split(":", 1)[1]

        # Guardamos el ID como entero cuando sea posible.
        try:
            liga_id = int(liga_id)
        except ValueError:
            pass
        ...
        # IMPORTANTE:
        # El nombre se obtiene de la cuenta actual, no del callback.
        # Así no dependemos de nombres codificados en callback_data.
        ligas = obtener_ligas()

        liga_encontrada = None
        for liga_item in ligas:
            if str(liga_item.get("id")) == str(liga_id):
                liga_encontrada = liga_item
                break

        if liga_encontrada is None:
            await query.edit_message_text(
                "❌ Esa liga ya no está disponible. Usa /liga de nuevo."
            )
            return
        ...
        # Sustituimos explícitamente el contexto de esta conversación.
        context.user_data["liga"] = liga_id
        context.user_data["liga_nombre"] = liga_nombre

    except Exception as e:
        log.exception("ERROR ELEGIR LIGA")
        await query.edit_message_text(
            f"❌ No se pudo seleccionar la liga:\n{e}"
        )
        return
...
async def informe(update, context):
    ...
    try:
        ...
        for manager, datos in managers:
            ...
            texto += (
                f"👤 {manager}\n"
                f"🟢 Compras: {numero_compras} ({compras:,.0f}€)\n"
                f"🔴 Ventas: {numero_ventas} ({ventas:,.0f}€)\n"
                f"{emoji} Saldo: {saldo:,.0f}€\n\n"
            )

        await enviar_largo(update, texto)

    except Exception as e:
        log.exception("ERROR INFORME")
        await update.message.reply_text(
...
async def mercado24(update, context):
    ...
    try:
        ...
        log.exception("ERROR MERCADO 24H")
        await update.message.reply_text(
